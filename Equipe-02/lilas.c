...
int incAluno(TAluno registro, TListAlunos *Tlista) {
  ...
  if (Tlista->eOrd == 0) {
    ...
    if (Tlista->tam >= (Tlista->cap - 1))
      return 0;
    else {
      // Adiciona o registro na ultima posição da lista e aumenta seu tamanho
      Tlista->aluno[Tlista->tam] = registro;
      Tlista->tam++;
    ...
    // Verifica se a lista está ou não cheia
    if (Tlista->tam >= (Tlista->cap))
      return 0;
    else {
      // Insertion sort

      int posicao = Tlista->tam;
      ...
      // menor
      while (posicao > 0 &&
             Tlista->aluno[posicao - 1].numMatricula > registro.numMatricula) {
        Tlista->aluno[posicao] = Tlista->aluno[posicao - 1];
        posicao--;
      }

      // Adiciona o registro na posição vaga e aumenta o tamanho da lista
...
int remAluno(TAluno *Taluno, TListAlunos *Tlista) {
  ...
  if (Tlista->eOrd) {
    if (posicao != Tlista->tam) {
      for (j = posicao; j < (Tlista->tam - 1); j++)
        Tlista->aluno[j] = Tlista->aluno[j + 1];
        Tlista->tam--;
        return TRUE;
    } else {
      return FALSE;
  ...
  if (listaB->eOrd) {
    for (int i = 0; i < listaA->tam; i++) {
      ...
      if (listaA->aluno[i].numMatricula == listaB->aluno[pos].numMatricula) {
        // se a lista for ordenada a busca retorna onde ele esta ou caso não
        // esteja onde ele deveria estar então é só comparar se o elemento de A
        // que eu mandei procurar é igual ao que B[pos] que
        // a função me retornou, caso sejam iguais
        listaC->aluno[listaC->tam] = listaA->aluno[i];
        listaC->tam++;
        // o inter.tam marca a posição onde o próximo elemento será colocado, e
        // sempre que for colocado o tam é incrementado
      }
    }
  } else {
    */
